...
class _TeamDatasets(PokemonSets):
    ...
    def get_all_possible_move_combinations(self, pkmn: Pokemon):
        pkmn_movesets = {}
        raw_pkmn_sets = self.get_raw_pkmn_sets_from_pkmn_name(pkmn.name)
        for pkmn_set, count in raw_pkmn_sets.items():
            pkmn_moves = tuple(sorted(pkmn_set.split("|")[5:]))
            pkmn_movesets[pkmn_moves] = pkmn_movesets.get(pkmn_moves, 0) + count
        return pkmn_movesets
